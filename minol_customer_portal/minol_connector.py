...
class MinolConnector:
    ...
    def get_user_tenants(self):
        ...
        try:
            ...
            if 'application/json' not in content_type:
                ...
                # Session wahrscheinlich abgelaufen - erzwinge Re-Login
                self._authenticated = False
                logger.info("Session appears to be invalid, marking as unauthenticated")
                raise ValueError("Invalid session - not logged in")
            ...
            # Session abgelaufen
            self._authenticated = False
            raise ValueError("Session expired - JSON decode error")
        except Exception as e:
    ...
    def fetch_em_data(self, timeline_start, timeline_end, cons_type="HZKWH", dlg_key="100KWH"):
        ...
        try:
            ...
            # DEBUG: Zeige alle Keys in der Response
            logger.debug(f"Response keys: {list(json_response.keys())}")
            if "dashboardHeader" in json_response:
                logger.debug(f"dashboardHeader value: '{json_response['dashboardHeader']}'")
            else:
                logger.debug("dashboardHeader NOT in response keys!")
    ...
    def _process_consumption_data(self, raw_data, consumption_type, timeline_start, timeline_end):
        ...
        Returns:
            dict: Processed data with by_room, overall timeline, total_consumption, and billing_period
        """
        processed = {
            "by_room": [],
            "timeline": [],
            "total_consumption": 0.0,
            "billing_period": None  # Abrechnungsperiode aus dashboardHeader
        }
        
        # Extrahiere Abrechnungsperiode aus dashboardHeader
        if "dashboardHeader" in raw_data and raw_data["dashboardHeader"]:
            processed["billing_period"] = raw_data["dashboardHeader"]
    ...
    def get_billing_period(self) -> Optional[str]:
        """
        Holt den dashboardHeader vom Dashboard.
        Der dashboardHeader wird nur zurückgegeben wenn die Dashboard-Ansicht (keine Raumdaten) abgerufen wird!
        
        Returns:
            str: Abrechnungsperiode z.B. "09.2025 - 01.2026" oder None
        """
        ...
        try:
            # Dashboard-Abruf: Aktuelle Periode (12 Monate)
            ...
            logger.info(f"Fetching dashboard header...")
            
            url = f"{self.base_url}/minol.com~kundenportal~em~web/rest/EMData/readData"
            
            # WICHTIG: Dashboard-View Parameter (table wird leer sein, aber dashboardHeader kommt!)
            payload = {
                "userNum": self.user_num,
                "layer": "NE",  # NE = Nutzerebene (wichtig!)
                "scale": "CALMONTH",
                "chartRefUnit": "ABS",
                "refObject": "DIN_AVG",
                "consType": "HZKWH",
                "dashBoardKey": "PE",  # PE = Dashboard
                "timelineStart": timeline_start,
                "timelineStartTxt": f"{timeline_start[4:]}.{timeline_start[:4]}",
                "timelineEnd": timeline_end,
                "timelineEndTxt": f"{timeline_end[4:]}.{timeline_end[:4]}",
            ...
            logger.debug(f"Dashboard request payload: {json.dumps(payload, indent=2)}")
            ...
            logger.debug(f"Dashboard response keys: {list(data.keys())}")
            logger.debug(f"dashboardHeader: '{data.get('dashboardHeader', 'NOT FOUND')}'")
            logger.debug(f"table length: {len(data.get('table', []))}")
            
            if "dashboardHeader" in data and data["dashboardHeader"]:
                billing_period = data["dashboardHeader"]
                logger.info(f"✅ Billing period from dashboard: {billing_period}")
                return billing_period
            else:
                logger.warning("⚠️ dashboardHeader ist leer oder nicht vorhanden")
                logger.debug(f"Full response: {json.dumps(data, indent=2)[:500]}")
                return None
                
        except Exception as e:
            logger.error(f"Error fetching billing period: {e}", exc_info=True)
            return None
